#ifndef RT_POINT_SHADOWS_PASS_H
#define RT_POINT_SHADOWS_PASS_H
...
{
    class RTPointShadowPass : public Graph::Pass
    {
    public:
        RTPointShadowPass(const std::string& name, Core::ResourceManager* resManager, VkExtent2D extent)
            : Pass(name), m_resourceManager(resManager), m_Extent(extent)
        {
            m_descriptorSet = Core::DescriptorBuilder(m_resourceManager->GetContext())
                ...
                .addLayoutBinding(2, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_RAYGEN_BIT_KHR)          // Shadow Mask Array (image2DArray)
                .build(Core::MAX_FRAMES_IN_FLIGHT);

            Core::RayTracingPipelineConfig rtConfig{};
            rtConfig.raygenShader = "shaders/RTPointShadow.rgen.spv";
        ...
        Graph::RGHandle GetShadowMaskArray() const { return m_shadowMaskArray; }

    private:
        Core::ResourceManager* m_resourceManager = nullptr;
        Graph::RGHandle m_shadowMaskArray;
