    ...
    : Pass(name), m_resourceManager(resManager), m_Extent(extent)
{
    ...
    rtConfig.raygenShader = "shaders/UnguidedRTAO.rgen.spv";
...
Render::Pass::UnguidedRTAOPass::~UnguidedRTAOPass()
{
...
void Render::Pass::UnguidedRTAOPass::Setup(Graph::RenderGraphBuilder& builder)
{
...
void Render::Pass::UnguidedRTAOPass::Execute(const RenderTypes::RenderContext& context, Render::Graph::RenderGraph& graph)
{
    Core::ImageBuilder::Image* maskImage = context.resourceManager->GetTexture(graph.GetResource(m_aoMask).physicalTexture);
