#include "RTPointShadowPass.h"
#include "../RenderGraph/RenderGraph.h"

void Render::Pass::RTPointShadowPass::Setup(Graph::RenderGraphBuilder& builder)
{
    Core::TextureDesc maskDesc{};
    maskDesc.name = "RT_PointShadowMask";
    ...
    maskDesc.arrayLayers = Core::MAX_POINT_LIGHTS; 

    m_shadowMaskArray = builder.CreateTexture(maskDesc,true);

    builder.AddDependency(m_shadowMaskArray, Graph::AccessType::ComputeShaderWrite);
...
void Render::Pass::RTPointShadowPass::Execute(const RenderTypes::RenderContext& context, Render::Graph::RenderGraph& graph)
{
    Core::ImageBuilder::Image* maskImage = context.resourceManager->GetTexture(graph.GetResource(m_shadowMaskArray).physicalTexture);
